...
- Поле подписи (`edit/SignaturePad.tsx`) после жалоб с телефона: «Готово»
  продублирована в шапке панели (низ панели уходит за экран при клавиатуре,
  альбомной ориентации, зуме); высота холста фиксирована в px (привязка к
  dvh заставляла рисунок плыть); пунктирная линия-опора лежит под холстом и
  в PNG не попадает; штрих привязан к одному pointerId (второй палец,
  ладонь, большой палец на краю не начинают и не обрывают линию; перо
  перехватывает у ладони; потеря захвата без pointerup не подвешивает
  холст); getCoalescedEvents для плавной линии; non-passive touchstart/
  touchmove preventDefault как страховка от прокрутки; при повороте снимок
  восстанавливается 1:1, уменьшается только если чернила не влезают.
  `Drawer` держит onClose в ref, чтобы эффект не перезапускался на каждый
  рендер родителя. Линия-опора совпадает с линией в акте: PNG режется по
  чернилам только по ширине, по высоте всегда от верха холста до 32 px под
  линией (линия на доле 156/188 высоты), а генератор ставит картинку так,
  чтобы эта доля легла на линию подписи (`sigBaseline` в generator.go).
- Имя файла PDF уникально для каждой генерации (`act_{номер}_{время}.pdf`):
  раньше все PDF акта писались в один файл и удаление любого документа
  ломало скачивание остальных.
